        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"> {{trans('app.price')}} </li>
                </ol>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive m-t-40">
                            <table id="myTable" class="table display table-bordered table-striped">
                                ...
                                <tr>
                                    <td>{{$id}}</td>
                                    <td>{{$price[$i]->village}}</td>
                                    <td>{{$price[$i]->price}}</td>
                                    <td>
                                        {{--@permission('تعديل حساب')--}}
                                        <a href="{{route('price.edit',$price[$i]->id)}}" class="btn btn-warning" >
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{--@endpermission--}}
                                        {{--@permission('حذف حساب')--}}
                                        <a><form action="{{route('price.destroy',$price[$i]->id)}}" style="display: inline;" method="POST" accept-charset="utf-8">
                                                {{csrf_field()}}
                                                {{method_field('DELETE')}}
