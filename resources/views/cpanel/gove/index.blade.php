        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"> {{trans('app.gove')}} </li>
                </ol>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive m-t-40">
                            <table id="myTable" class="table display table-bordered table-striped">
                                ...
                                <tr>
                                    <th>{{trans('app.id')}}</th>
                                    <th> {{trans('app.gove')}}  </th>
                                    <th>{{trans('app.controll')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php $id=1;?>
                                @foreach($gove as $goves)
                                    <tr>
                                    <td>{{$id}}</td>
                                    <td>{{$goves->gove}}</td>
                                    <td>
                                        {{--@permission('تعديل حساب')--}}
                                        <a href="{{route('gove.edit',$goves->ids)}}" class="btn btn-warning" >
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        {{--@endpermission--}}
                                        {{--@permission('حذف حساب')--}}
                                        <a><form action="{{route('gove.destroy',$goves->ids)}}" style="display: inline;" method="POST" accept-charset="utf-8">
                                                {{csrf_field()}}
                                                {{method_field('DELETE')}}
