                ...
                <div class="row page-titles">
                    <div class="col-md-5 col-8 align-self-center">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{url('admin/captin')}}">{{trans('app.captin')}} </a> </li>
                            <li class="breadcrumb-item active">{{trans('app.edit')}}  </li>
                        </ol>
                    </div>
                </div>
                <div class="box box-primary">
                    <form action="{{route('captin.update',$captin->id)}}" method="post" role="form" accept-charset="utf-8" enctype="multipart/form-data">
                        {{method_field('PATCH')}}
                        {{csrf_field()}}
                        <div class="card card-body col-md-6">
                            <div class="form-group">
                                <label for="name">{{trans('app.name')}} <span style="color:red;">*</span>:</label>
                                <input name="name" value="{{$captin->name}}" type="text" class="form-control" id="name" placeholder="العميل " required />
                            </div>
                            <div class="form-group">
                                <label for="name"> {{trans('app.telephone')}} <span style="color:red;">*</span>:</label>
                                <input name="telephone" value="{{$captin->telephone}}" type="text" class="form-control" id="name" placeholder="رقم التليفون " required />
                            </div>
                            <div class="form-group">
                                <label for="name">{{trans('app.email')}}  <span style="color:red;">*</span>:</label>
                                <input name="email" type="email" value="{{$captin->email}}" class="form-control" id="name" placeholder="البريد الالكتروني " required />
                            </div>
                            <div class="form-group">
