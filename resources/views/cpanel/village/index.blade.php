        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">     {{trans('app.village')}}</li>
                </ol>
            </div>
                        ...
                        <div class="table-responsive m-t-40">
                            <table id="myTable" class="table display table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>  {{trans('app.village')}} </th>
                                    <th> {{trans('app.controll')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @for($i=0;$i<count($viallage);$i++)
                                    <tr>
                                        <td>1</td>
                                        <td>{{$viallage[$i]->area}}</td>
                                        <td>{{$viallage[$i]->village}}</td>
                                        <td>
                                            {{--@permission('تعديل حساب')--}}
                                            <a href="{{route('village.edit',$viallage[$i]->ids)}}" class="btn btn-warning" >
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            {{--@endpermission--}}
                                            {{--@permission('حذف حساب')--}}
                                            <a><form action="{{route('village.destroy',$viallage[$i]->ids)}}" style="display: inline;" method="POST" accept-charset="utf-8">
                                                    {{csrf_field()}}
                                                    {{method_field('DELETE')}}
